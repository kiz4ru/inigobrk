/* ============================================================================
   fluid.js — Simulación de fluidos WebGL como fondo del hero
   ----------------------------------------------------------------------------
   Adaptado de "WebGL Fluid Simulation" de Pavel Dobryakov (licencia MIT).
   Cambios: fondo oscuro (config.json), canvas acotado al hero, carga de
   configuración externa, respeto de prefers-reduced-motion, sólo tema oscuro.
   ========================================================================== */
(() => {
  "use strict";

  const canvas = document.querySelector(".hero__fluid");
  if (!canvas) return;

  const REDUCED = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  if (REDUCED) { canvas.classList.add("is-off"); return; }

  /* -------- Configuración por defecto (se fusiona con config.json) -------- */
  let config = {
    SIM_RESOLUTION: 128,
    DYE_RESOLUTION: 1024,
  ...
  function rectPos(clientX, clientY) {
  ...
  canvas.addEventListener("mousedown", e => {
    const p = rectPos(e.clientX, e.clientY);
    let pointer = pointers.find(pt => pt.id === -1) || pointers[0];
  ...
  canvas.addEventListener("mousemove", e => {
    ...
    const p = rectPos(e.clientX, e.clientY);
    updatePointerMoveData(pointer, p.x, p.y);
  });
  ...
  canvas.addEventListener("touchstart", e => {
    ...
    for (let i = 0; i < touches.length; i++) {
      const p = rectPos(touches[i].clientX, touches[i].clientY);
      updatePointerDownData(pointers[i + 1], touches[i].identifier, p.x, p.y);
    }
  }, { passive: false });
  ...
  function generateColor() {
    let c;
    if (config.RANDOM_COLORS) c = HSVtoRGB(Math.random(), 1.0, 1.0);
    else c = HSVtoRGB(config.SPLAT_HUE / 360, 1.0, 1.0);
    c.r *= 0.15;
    c.g *= 0.15;
    c.b *= 0.15;
    return c;
  ...
  function syncThemeColor() {
    config.BACK_COLOR = Object.assign({}, THEME_BG[currentTheme()]);
  }
  // El fondo se redibuja cada frame desde config.BACK_COLOR: basta con actualizarlo.
  new MutationObserver(syncThemeColor).observe(document.documentElement, {
    attributes: true, attributeFilter: ["data-theme"]
  });

  /* --------------------- Arranque: carga config.json --------------------- */
  fetch("config.json")
    .then(r => r.ok ? r.json() : Promise.reject())
    .then(json => { config = Object.assign(config, json); })
